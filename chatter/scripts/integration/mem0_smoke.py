...
def main() -> int:
    ...
    base_url = os.getenv("MEM0_BASE_URL", "https://api.mem0.ai").rstrip("/")
    ...
    try:
        add_resp = _request("POST", f"{base_url}/v1/memories/", headers, add_payload, timeout)
    ...
    try:
        search_resp = _request("POST", f"{base_url}/v2/memories/search", headers, search_payload, timeout)
        ...
        if created_id:
            try:
                _request("DELETE", f"{base_url}/v1/memories/{created_id}", headers, None, timeout)
            except Exception:
                pass

...
const schemaMap = {
  MemoryExtraction: path.join(repoRoot, "packages/protocol/jsonschema/memory_extraction.schema.json"),
  DriftReflection: path.join(repoRoot, "packages/protocol/jsonschema/drift_reflection.schema.json"),
};
...
const validateFixtures = async (name, version, schema, ajv) => {
  ...
  if (!baseExists) {
    console.warn(`[WARN] Fixture directory missing for ${name} ${version}: ${baseDir}`);
  ...
  for (const { label, directory, expectValid } of scenarios) {
    ...
    if (files.length === 0) {
      console.warn(`[WARN] ${label} fixtures missing or empty for ${name} ${version}: ${directory}`);
      continue;
    }
    ...
    for (const filePath of files) {
      try {
        ...
        if (isValid && expectValid) {
          ...
          console.error(`  [FAIL] ${filePath}: validation failed (${ajv.errorsText(validator.errors)})`);
          failed += 1;
        }
      } catch (error) {
